...
var SenseSearchInput = function() {
    var inputTimeout;
    var ignoreKeys = [ 16, 27 ];
    var reservedKeys = [ 9, 13, 38, 40, 39, 37, 32, 33, 34 ];
    var Key = {
        BACKSPACE: 8,
        ESCAPE: 27,
        CONTROL: 17,
    ...
    SenseSearchInput.prototype = Object.create(Object.prototype, {
        ...
        nlpModel: {
            value: {
                ...
                negationMap: [ "not", "ignore", "ignoring", "exclude", "excluding", "without" ],
                defaultFunction: "sum",
                currencySymbol: "£",
                misc: [ "by", "as", "is", "was", "were", "me", "not", "ignore", "the", "in", "of", "a", "and", "what", "show" ],
                comparatives: {
                    more: ">",
                    less: "<",
                    greater: ">",
                ...
                conditionals: [ "where", "for", "is", "from" ],
                sorting: [ "order", "ordered", "sort", "sorted" ],
                sortorder: {
                    asc: 1,
                    ascending: 1,
                    lowest: 1,
        ...
        transformAssociations: {
            value: function(associations) {
                ...
                if (associations.qSearchTermsMatched[0]) {
                ...
                for (var i = 0; i < transformation2.length; i++) {
                    ...
                    if (itemIndex === -1) {
                        ...
                        transformation3.push({
                            id: tempTerms.join("_"),
                            terms: transformation2[i].terms,
                            fields: []
                        });
                    }
                ...
                if (!term.queryTag) {
                    if (this.nlpModel.functionMap[termText]) {
                        ...
                        if (this.nlpTerms[currentTermIndex - 1] && this.nlpTerms[currentTermIndex - 1].queryTag === "sort by") {
                            ...
                            term.senseInfo = {
                                order: this.nlpModel.sortorder[termText]
                            };
                        }
        ...
        showAssociations: {
            value: function() {
                ...
                for (var i = 0; i < this.associations.qSearchTermsMatched.length; i++) {
                    var termsMatched = this.associations.qSearchTermsMatched[i];
                    for (var j = 0; j < termsMatched.length; j++) {
                        html += "<li class='sense-search-association-item' data-index='" + j + "'>";
                        var associationSummary = [];
                        for (var k = 0; k < termsMatched[j].qFieldMatches.length; k++) {
                            var extraStyle = "";
        ...
        nlpViz: {
            value: function() {
                    ...
                    switch (this.nlpTerms[t].senseGroup) {
                      case "exp":
                        ...
                        measures[measureName] = {
                            field: this.nlpTerms[t].senseInfo.field
                        };
                      ...
                      case "dim":
                        ...
                        dimensions[dimensionName].qDef.qSortCriterias = [ {
                            qSortByAscii: 1
                        } ];
                ...
                if (sortCount == 0 && hDef.qHyperCubeDef.qDimensions && hDef.qHyperCubeDef.qDimensions.length > 0) {
                    if (time.length > 0) {
                        ...
                        if (hDef.qHyperCubeDef.qMeasures.length > 0) {
                            ...
                            var colOrder = new Array(hDef.qHyperCubeDef.qDimensions.length + hDef.qHyperCubeDef.qMeasures.length).fill().map(function(item, index) {
                                return index;
                            });
    ...
    function highlightAssociationValue(match) {
        ...
        if (match.qRanges[0] != null) {
            matchValue = {
                qText: match.qText.substring(match.qRanges[0].qCharPos, match.qRanges[0].qCharPos + match.qRanges[0].qCharCount)
            };
        }
        ...
        text = text.join("").replace(/<(?!\/?span(?=>|\s.*>))\/?.*?>/gim, "");
        return {
            text: text,
            elem: elemNum,
        ...
        getHyperCubeData: {
            value: function(callbackFn) {
                ...
                senseSearch.exchange.getLayout(this.handle, function(response) {
                    ...
                    senseSearch.exchange.ask(that.handle, "GetHyperCubeData", [ "/qHyperCubeDef", [ {
                        qTop: that.pageTop,
                        qLeft: 0,
                        qHeight: that.pageSize,
                        qWidth: that.fields.length
                    } ] ], function(response) {
        ...
        Exchange.prototype = Object.create(Object.prototype, {
            ...
            askCapabilityAPI: {
                value: function(handle, method, args, callbackFn) {
                    ...
                    var conn;
                    try {
                        this.connection.__enigmaSession.rpc.send({
                            handle: handle,
                            method: method,
                            params: args
    ...
    SenseSearch.prototype = Object.create(Object.prototype, {
        ...
        search: {
            value: function(searchText, searchFields, mode, context) {
                ...
                this.exchange.ask(this.appHandle, "SearchAssociations", [ {
                    qContext: context,
        ...
        selectAssociations: {
            value: function(searchFields, resultGroup, context) {
                context = context || this.context || "LockedFieldsOnly";
                var that = this;
                that.exchange.ask(that.appHandle, "SelectAssociations", [ {
                    qContext: context,
                    qSearchFields: searchFields
                }, that.terms, resultGroup ], function(response) {
        ...
        suggest: {
            value: function(searchText, suggestFields, context) {
                ...
                this.exchange.ask(this.appHandle, "SearchSuggest", [ {
                    qContext: context || this.context || "LockedFieldsOnly",
                    qSearchFields: suggestFields
                }, searchText.split(" ") ], function(response) {
            ...
            value: function(def) {
                ...
                if (this.exchange.connectionType == "CapabilityAPI") {
                    ...
                    if (def.qInfo.qType == "boxplot") {
                        ...
                        defOptions.boxplotDef = {
                            qHyperCubeDef: defOptions.qHyperCubeDef
                        };
                    ...
                    this.exchange.app.visualization.create(def.qInfo.qType, [], defOptions).then(function(chart) {
                        ...
                        if (def.qInfo.qType == "table") {
                            var colOrder = new Array(defOptions.qHyperCubeDef.qDimensions.length + defOptions.qHyperCubeDef.qMeasures.length).fill().map(function(item, index) {
                                return index;
                            });
                            var patchDefs = [ {
                                qOp: "replace",
                        ...
                        if (response.id >= that.pendingChart) {
                            if (that.exchange.connectionType == "Enigma") {
                                ...
                                that.exchange.ask(response.result.qReturn.qHandle, "GetLayout", [], function(layoutResponse) {
                                    that.chartResults.deliver({
                                        model: response.result.qReturn,
                                        layout: layoutResponse.result.qLayout
                                    });
                                });
                            }
                        }
        ...
        getAppFields: {
            value: function(cardinalityLimit, includeMeasures) {
                ...
                that.exchange.ask(that.appHandle, "CreateSessionObject", [ {
                    qInfo: {
                        qType: "FieldList"
                    },
                    qFieldListDef: {
            ...
            value: function(fieldData, cardinalityLimit) {
                ...
                for (var i = 0; i < fieldData.dimensions.length; i++) {
                    ...
                    if (fieldData.fields[i].qCardinal > cardinalityLimit) {
                        ...
                        this.appFieldsByTag.$possibleMeasure[fieldName] = {
